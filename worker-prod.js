...
(() => {
  ...
  const bindFeedback=()=>{const card=ensureFeedback();if(!card||card.dataset.bound)return;if(card.dataset.bound='1',true){card.addEventListener('click',e=>{const button=e.target.closest('[data-feedback],[data-reason]');if(!button)return;if(button.dataset.feedback==='yes'){card.querySelector('.feedback-actions').innerHTML='<div class="feedback-thanks show">Thanks — that helps us improve the estimator.</div>';}
      if(button.dataset.feedback==='no')card.querySelector('.feedback-reasons').classList.add('show');
      if(button.dataset.reason){card.querySelector('.feedback-actions').innerHTML='<div class="feedback-thanks show">Thanks — that helps us improve the estimator.</div>';card.querySelector('.feedback-reasons').classList.remove('show');}
    });}};
